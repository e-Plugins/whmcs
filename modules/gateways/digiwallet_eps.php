 ...
 * @return array
 */
function digiwallet_eps_MetaData()
{
    return array(
        'DisplayName' => 'Digiwallet - EPS',
 ...
 * @return array
 */
function digiwallet_eps_config()
{
    ...
    return array(
        ...
        'FriendlyName' => array(
            'Type' => 'System',
            'Value' => sprintf($dwLang['digiwallet']['default_method_name'], 'EPS'),
        ),
        'rtlo' => array(
 ...
 * @return string
 */
function digiwallet_eps_link($params)
{
...
add_hook('ShoppingCartValidateCheckout', 1, function($vars) {
    ...
    if($vars['paymentmethod'] == $gatewayModuleName) {
        ...
        if (empty($gatewayParams['rtlo'])) {
            return [
                sprintf($dwLang['digiwallet']['error_missing_rtlo'], 'EPS'),
            ];
        }
        if (empty($gatewayParams['token'])) {
            return [
                sprintf($dwLang['digiwallet']['error_missing_token'], 'EPS'),
            ];
        }
    }
});