 ...
 * @return array
 */
function digiwallet_sofort_MetaData()
{
    return array(
        'DisplayName' => 'Digiwallet - Sofort',
...
function digiwallet_sofort_config()
{
    ...
    return array(
        ...
        'FriendlyName' => array(
            'Type' => 'System',
            'Value' => sprintf($dwLang['digiwallet']['default_method_name'], 'Sofort'),
        ),
        'rtlo' => array(
 ...
 * @return string
 */
function digiwallet_sofort_link($params)
{
 ...
 */
function digiwallet_sofort_refund($params)
{
...
add_hook('ShoppingCartValidateCheckout', 1, function($vars) {
    ...
    if($vars['paymentmethod'] == $gatewayModuleName) {
        ...
        if (empty($gatewayParams['rtlo'])) {
            return [
                sprintf($dwLang['digiwallet']['error_missing_rtlo'], 'Sofort'),
            ];
        }
    }
});